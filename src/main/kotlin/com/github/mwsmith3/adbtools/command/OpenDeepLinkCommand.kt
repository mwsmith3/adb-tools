...
class OpenDeepLinkCommand(private val project: Project, private val deepLinkData: DeepLinkData) : Command<Unit> {
    private val deepLink = "\"${deepLinkData.deepLink.link}${deepLinkData.param}${deepLinkData.value}\""

    override val command: String
        get() = "am start -a android.intent.action.VIEW -d $deepLink ${deepLinkData.deepLink.packageName}"
    ...
    override fun run(device: IDevice) {
        ...
        if (deepLinkData.attachDebugger) {
            Debugger(project, device, deepLinkData.deepLink.packageName).attach()
        }
    }
}